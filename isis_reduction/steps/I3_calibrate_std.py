...
for night in obsData['data_location']['night_list']:
    ...
    for arm_color in colors:
        ...
        night_cenWaves = night_conf[f'night_{arm_color}_cenwaves_array']

        # Loop through the objects
        for objIdx, objName, objCenWave in zip(obj_idx, obj_names, obj_cenwaves):

            # Standard stars
            if objName in std_stars:
                input_file = f'{pr.reducDf.loc[objIdx].file_name}'
                output_file = f'{run_folder}/{input_file.replace(".fits", "_f.fits")}'
                sens_file = f'{run_folder}/{objName}_{arm_color}_sens.fits'
