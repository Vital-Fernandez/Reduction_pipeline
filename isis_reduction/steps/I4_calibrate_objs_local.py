    ...
    for arm_color in colors:
        ...
        for objIdx, objName, objCenWave in zip(obj_idx, obj_names, obj_cenwaves):
            ...
            if objName in objs:
                ...
                idcs_objStar = (df_targets.frame_tag.isin(std_stars)) & \
                               (df_targets.CENWAVE.apply(np.isclose, b=objCenWave, atol=4)) & \
                               (df_targets.valid_file)

                name_stars = df_targets.loc[idcs_objStar].frame_tag.values
                cenwave_stars = df_targets.loc[idcs_objStar].CENWAVE.values
                print(f'{objName} ({objCenWave}) -> \t {name_stars} ({cenwave_stars})')

                # For every compatible calibration run the calibration
                for i, calibStar in enumerate(name_stars):
                    sens_file = f'{run_folder}/{calibStar}_{arm_color}_sens.fits'
                    output_file = f'{run_folder}/{input_file.replace(".fits", "")}' + f'_f-{calibStar}.fits'
